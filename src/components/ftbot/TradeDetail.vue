...
<template>
  <div class="text-start grid grid-cols-[repeat(auto-fit,minmax(500px,1fr))] gap-4 px-2">
    <div class="">
      <h5 class="detail-header">{{ $t('tradeDetail.sectionGeneral') }}</h5>
      ...
      <ValuePair v-if="trade.is_open" :description="$t('tradeDetail.stake')">
        ...
        <template v-if="trade.trading_mode !== 'spot'">
          ({{ trade.leverage }}x)
          <span :title="$t('tradeDetail.positionValue')" class="italic">{{
      ...
      <ValuePair v-if="trade.is_open && trade.current_rate" :description="$t('tradeDetail.currentRate')">
        ...
        <span :title="$t('tradeDetail.currentValueHint')" class="italic">
      ...
      <ValuePair
        ...
        :description="
          trade.is_open ? $t('tradeDetail.currentProfit') : $t('tradeDetail.closeProfit')
        "
      >
      ...
      <details>
        ...
        <ValuePair :description="$t('tradeDetail.openFees')">
          ...
          <span v-if="trade.quote_currency !== trade.fee_open_currency">
            ({{ $t('tradeDetail.openFeesIn', { currency: trade.fee_open_currency }) }})
          </span>
          ({{ formatPercent(trade.fee_open) }})
        </ValuePair>
        <ValuePair v-if="trade.fee_close_cost && trade.fee_close" :description="$t('tradeDetail.feesClose')">
    ...
    <div class="mt-2 lg:mt-0">
      <h5 class="detail-header">{{ $t('tradeDetail.stoplossSection') }}</h5>
      ...
      <details v-if="trade.orders">
        <summary>
          {{ $t('tradeDetail.orders') }} {{ trade.orders.length > 1 ? `[${trade.orders.length}]` : '' }}
        </summary>
        <div
        ...
        >
          ...
          {{ $t('tradeDetail.orderAt') }} <b>{{ formatPrice(order.safe_price) }}</b> |
          <span v-if="order.remaining && order.remaining !== 0" :title="$t('tradeDetail.remaining')">
            {{ formatPrice(order.remaining, 8) }} /
          </span>
          <span :title="$t('tradeDetail.filled')">{{ formatPrice(order.filled ?? 0, 8) }}</span>
